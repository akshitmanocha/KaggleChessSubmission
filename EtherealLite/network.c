...
void initPKNetwork() {
    ...
    // Read input layer weights and biases in half precision
    for (int i = 0; i < PKNETWORK_LAYER1; i++) {
        for (int j = 0; j < PKNETWORK_INPUTS; j++) {
            uint16_t half = read_half(file);
            PKNN.inputWeights[j][i] = half_to_float(half);
        }
        uint16_t half = read_half(file);
        PKNN.inputBiases[i] = half_to_float(half);
    }

    // Read output layer weights and biases in half precision
    for (int i = 0; i < PKNETWORK_OUTPUTS; i++) {
        for (int j = 0; j < PKNETWORK_LAYER1; j++) {
            uint16_t half = read_half(file);
            PKNN.layer1Weights[i][j] = half_to_float(half);
        }
        uint16_t half = read_half(file);
        PKNN.layer1Biases[i] = half_to_float(half);
    }
